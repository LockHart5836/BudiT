...
const DarkMode = {
  toggle() {
    ...
    localStorage.setItem('dev.finances:darkMode', document.body.classList.contains('dark-mode'));
  },
  load() {
    if (JSON.parse(localStorage.getItem('dev.finances:darkMode'))) {
...
const DOM = {
  ...
  innerHTMLTransaction(transaction, index) {
    ...
    const html = `
      ...
      <td>
          <img onclick="Transaction.remove(${index})" src="./assets/x-circle.svg" class="remove-transaction" alt="Remove transaction">
      </td>
      `;
    return html;
  },
...
const Utils = {
  ...
  formatAmount(value) {
    ...
    // check if user adds a - (minus sign) instead select "Income" and "Expense" Buttons
    if (value < 0) {
      value = -value;
    }
...
const Form = {
  ...
  validateField() {
    ...
    if (
      ...
      throw new Error("Please fill in field description!");
    }
  },
